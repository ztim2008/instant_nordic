...
class actionNordicblocksBlockCssPublish extends cmsAction {
    ...
    public function run($block_id = 0) {
        ...
        $publish_result = $this->model->publishBlockCssOverlayState(
            (int) ($block['id'] ?? 0),
            (string) ($block['type'] ?? ''),
            $expected_version,
        ...
        echo json_encode($publish_result, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
        exit;
    }
}