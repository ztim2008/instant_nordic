(function($) {
    ...
    if ($('.onebuilder-header').length) {
        let $body = $('body'),
            $header = $('.onebuilder-header'),
            $hamburger = $('.hamburger'),
        ...
        $hamburger.click(function(ev) {
            ...
            $(this).toggleClass('is-active');
            $menu = $(this).closest('.onebuilder-header').find('.js-menu');
            $header.toggleClass('is-open');
            if ($(this).hasClass('is-active')) {
        ...
        if ($('.js-center-logo').length) {
            ...
            function moveLogoToMenu() {
                ...
                let menuLength = $('.onebuilder-header .main-menu > .menu-item').length;
                if (0 == menuLength % 2) {
                    $logo.insertAfter('.onebuilder-header .main-menu > .menu-item:nth-child(' + Math.floor(menuLength / 2) + ')');
                }
                logoIsMoved = true;
            }
    ...
    function initSwiper() {
        ...
        $('.swiper-container').each(function() {
            ...
            if ('responsive' == slidesPerViewVal) {
                slidesPerViewVal = updateSlidesPerView($t);
            } else if ('auto' == slidesPerViewVal) {} else {
            ...
            swipers['swiper-' + index] = new Swiper('.swiper-' + index, {
                ...
                on: {
                    init: function() {
                        if (progBarVal === 'custom') {
                            $('.swiper-pagination-' + index).addClass('swiper-pagination-custom').append($('<div class="swiper-pagination-custom-line" style="transition-duration: ' + speedVal + 'ms"></div>'));
                        }
                    },
    ...
    initSwiper();
    $(".filter-construction .onebuilder-pf__filter").on("click", function() {
        var filter = $(this).data('filter');
        var slides = $(this).closest('.onebuilder-pf').find('.swiper-slide');
        var swiper_id = $(this).closest('.onebuilder-pf').find('.swiper-container').attr('id');
            ...
            if (!progBarView) {
                if (isScrolledIntoView(progBar)) {
                    ...
                    progBar.each(function() {
                        ...
                        $(this).find('.prog-bar-hldr').animate({
                            left: percentage + '%'
                        }, 2000);
                        $(this).find('.prog-bar-val').animate({
                            width: percentage + '%'
                        }, 2000);
                    });
                }
    ...
    $('.onebuilder-video a').on('click', function(e) {
        e.preventDefault();
        var videoWrapp = $(this).closest('.onebuilder-video');
    ...
    if ($('.onebuilder-content-block').length) {
        $('.onebuilder-content-block').parent().parent().each(function() {
            index = 0;
            $(this).find('.onebuilder-content-block--list-icon, .onebuilder-content-block--list, .onebuilder-content-block--slider').each(function(index) {
                index++;
    ...
    $('.onebuilder-content-block--bgImg').hover(function() {
        $(this).find('.onebuilder-content-block__info').slideDown(200);
    }, function() {
        $(this).find('.onebuilder-content-block__info').slideUp(200);
    });
    ...
    if ($('.js-typed-karma').length) {
        ...
        $(typedItem).each(function(index, el) {
            $(this).attr('id', 'js-typed-karma-' + index);
            $(this).attr('data-text', $(this).text()).text('');
        });
    ...
    if ($('.js-typed-karma-modern').length) {
        ...
        $(typedItem).each(function(index, el) {
            $(this).attr('id', 'js-typed-karma-modern-' + index);
            $(this).attr('data-text', $(this).text()).text('');
        });
    ...
    function productWidth() {
        shopItem = $('.onebuilder-products__item:first-child');
        shopItemWidth = $('.onebuilder-products__item:first-child').width();
    ...
    var blogMetro = $('.onebuilder-content--metro');
    if (blogMetro.length) {
    ...
    var titleArrDown = $('.onebuilder-titlebar__arrow-down');
    if (titleArrDown.length) {
        titleArrDown.each(function(index) {
            $(this).click(function(ev) {
                $('html, body').animate({
                    scrollTop: $(this).closest('.onebuilder-titlebar').offset().top + $(this).closest('.onebuilder-titlebar').height()
                }, 1000);
            });
        });
    }
    if ($('.onebuilder-pf').length) {
        var pfGrid = [];
        $('.onebuilder-pf').each(function(index) {
            if (!$(this).hasClass('onebuilder-pf--masonry')) {
                pfGrid[index] = $(this).find('.onebuilder-pf__container');
                pfGrid[index].isotope({
                    masonry: {
                        columnWidth: '.onebuilder-pf__item:not(.onebuilder-pf__item--height):not(.onebuilder-pf__item--width)'
                    }
                });
            } else {
                pfGrid[index] = $('.onebuilder-pf--masonry .onebuilder-pf__container').isotope({
                    itemSelector: '.onebuilder-pf__item',
                    percentPosition: true,
                    masonry: {
                        columnWidth: '.onebuilder-pf__item'
                    }
                });
            ...
            $('.onebuilder-pf__filter').on('click', function() {
                $('.onebuilder-pf__filter').removeClass('active');
        ...
        if ($('.onebuilder-pf--new').length) {
            var pfModal = $('.onebuilder-pf__modal');
            var pfImg = $('.onebuilder-pf__img');
            var pfModalImg = $('.onebuilder-pf__modal-content');
            pfImg.click(function(ev) {
                ev.stopPropagation();
                pfModal.css('display', 'flex');
                pfModalImg.attr('src', $(this).find('img').attr('src'));
            });
            var pfspan = $('.onebuilder-pf__close')[0];
    ...
    if ($('.js-grid').length) {
        var isotopeGrid = $('.js-grid').isotope({
    ...
    if ($('.onebuilder-pricing__detail--checked').length) {
        var itemChecked = $('.onebuilder-pricing__detail--checked'),
            labels = itemChecked.closest('.onebuilder-pricing').eq(0).siblings('.onebuilder-pricing--legend').eq(0).find('.onebuilder-pricing__detail'),
            labelArr = [];
        ...
        $('.onebuilder-pricing__detail--not-labeled').each(function() {
    ...
    if ($('.js-wdgt-post').length) {
        ...
        $btnList.children().click(function(ev) {
            ...
            $(this).addClass('active');
            $(this).closest('.js-wdgt-post').find('.js-wdgt-post-list').children().removeClass('active').eq($(this).index()).css('opacity', 0).addClass('active').animate({
                opacity: 1
            }, 300);
        });
    }
    ...
    var metroItem = $('.onebuilder-pf__container');
    if (metroItem.length) {
        ...
        $('.loading').on('click', function() {
            ...
            function customDelay() {
                metroItem.find('.onebuilder-pf__item').removeClass('hide');
    ...
    $('.load-more a').on('click', function() {
        $('.onebuilder-pf__item').removeClass('hide');
    ...
    if (shopItem.length) {
        $('.loading').on('click', function() {
            ...
            function customDelay() {
                shopItem.find('.onebuilder-products__item').removeClass('hide');
                $('.loading').addClass('hide');
            }
            setTimeout(customDelay, 1000);
        });
    ...
    $(".onebuilder-content-block--construction").mouseover(function() {
        $(".onebuilder-content-block--construction").removeClass('active');
        $(this).addClass('active');
    });
    ...
    $(window).scroll(function() {
        if ($(this).scrollTop() > 10) {
            $('.onebuilder-header-3__inner').addClass('onebuilder-header-3__fix');
        } else {
            $('.onebuilder-header-3__inner').removeClass('onebuilder-header-3__fix');
        }
    });
    ...
    $('.onebuilder-accordion__item:first-child .js-accordion').addClass('active');
    $('.onebuilder-accordion__item:first-child .onebuilder-accordion__panel').show();
    $(window).load(function() {
