...
function install_package() {
    // Core installer imports install.sql and registers component from manifest.
    return true;
}
...
function after_install_package() {
    ...
    try {
        NordicbuilderPackageMigrations::apply(
            cmsDatabase::getInstance(),
            __DIR__ . '/migrations'
        );
    } catch (Throwable $exception) {
        return 'Nordicbuilder migrations failed: ' . $exception->getMessage();
    }

    return true;
}

class NordicbuilderPackageMigrations {
    ...
    public static function apply($db, $migrations_dir) {
        ...
        foreach ($files as $file) {
            ...
            $db->query(
                "INSERT INTO `{#}nordicbuilder_migrations` (`version`, `description`, `applied_at`) VALUES ('%s', '%s', NOW())",
                [$version, $description]
            );
        }
    ...
    protected static function isApplied($db, $version) {
        ...
        return (int) $db->getRowsCount('nordicbuilder_migrations', $where) > 0;
    }
