...
class actionNordicbuilderShellEdit extends cmsAction {

	public function run($variant_key = null) {

		$lb_model = cmsCore::getModel('landingbuilder');

		$variant = $lb_model->getShellVariantByKey($variant_key);
		if (!$variant) {
		...
		if ($this->request->has('submit')) {
			...
			if (!$errors) {
				$variant = $lb_model->saveShellVariant($variant['key'], $data);
			...
			$variant = $lb_model->prepareShellVariant($variant['key'], $data);
		...
		return $this->cms_template->render('backend/shell_variant', [
			...
			'variant'      => $variant,
			'preview_rows' => $lb_model->getShellPreviewRows($variant),
			'catalog'      => $catalog,
			'form'         => $form,
			'errors'       => $errors ?? false
		]);
