...
<div class="lb-shell-variant-ui">
	<section class="lb-shell-panel">
		<div class="lb-admin-eyebrow">Shell Variant</div>
		<h3 class="lb-admin-title"><?php html($variant['title']); ?></h3>
		<p class="lb-admin-copy"><?php html($variant['description']); ?></p>
		<p class="lb-admin-copy">Область применения: <?php html($scope_titles[$variant['scope']] ?? $variant['scope']); ?><?php if ($variant['target_label']) { ?> | <?php html($variant['target_label']); ?><?php } ?> | Ключ варианта: <?php html($variant['key']); ?></p>
	</section>

	<div class="lb-shell-variant-grid">
		<section class="lb-shell-panel">
			<h4 class="lb-shell-panel__title">Preview shell slots</h4>
			<p class="lb-shell-panel__copy">Активные зоны уже участвуют в текущем variant, а приглушенные остаются выключенными на уровне shell-настроек.</p>
					...
					<div class="lb-slot-grid">
						<?php foreach ($row['cols'] as $col) { ?>
							<div class="lb-slot-card<?php if ($col['is_active']) { ?> is-active<?php } else { ?> is-muted<?php } ?>">
								...
								<div class="lb-slot-card__state"><?php echo $col['is_active'] ? 'Активен в shell' : 'Скрыт этим variant'; ?></div>
							</div>
						<?php } ?>
					</div>
				</div>
		...
		<div class="lb-shell-variant-ui">
			...
			<section class="lb-shell-panel">
				<h4 class="lb-shell-panel__title">Настройки variant</h4>
