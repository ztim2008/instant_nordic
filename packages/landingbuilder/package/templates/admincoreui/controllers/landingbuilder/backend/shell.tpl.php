...
$this->setPageTitle('Нордик: Каркас сайта');
$this->addBreadcrumb('Нордик');
$this->addBreadcrumb('Каркас сайта');
...
<div class="lb-shell-ui">
	<section class="lb-shell-hero">
		<div class="lb-admin-eyebrow">Каркас сайта</div>
		<h3 class="lb-admin-title">Варианты каркаса сайта</h3>
		<p class="lb-admin-copy">Здесь выбирается, как выглядят шапка, подвал, меню, первый экран и зоны вокруг контента. Это влияет на внешний вид ключевых страниц сайта.</p>
	</section>
		...
		<?php foreach ($variants as $variant) { ?>
			<article class="lb-shell-card">
				...
				<div class="lb-shell-chip-row">
					...
					<?php if (!empty($variant['is_system'])) { ?><span class="lb-shell-chip">встроенный</span><?php } ?>
				</div>
				<div class="lb-shell-card__meta">
					...
					<div class="lb-shell-kv">
						<span class="lb-shell-kv__label">Шапка / Подвал</span>
						<span class="lb-shell-kv__value">Шапка: <?php html($header_titles[$variant['header_variant']] ?? $variant['header_variant']); ?><br>Подвал: <?php html($footer_titles[$variant['footer_variant']] ?? $variant['footer_variant']); ?></span>
					</div>
					<div class="lb-shell-kv">
					...
					<div class="lb-shell-kv">
						...
						<span class="lb-shell-kv__value">Первый экран: <?php echo !empty($variant['show_hero']) ? 'включен' : 'выключен'; ?><br>До контента: <?php echo !empty($variant['show_before_content']) ? 'да' : 'нет'; ?><br>После контента: <?php echo !empty($variant['show_after_content']) ? 'да' : 'нет'; ?></span>
					</div>
					<div class="lb-shell-kv">
				...
				<a class="lb-shell-btn" href="<?php html($variant['edit_url']); ?>">Редактировать вариант</a>
			</article>
		<?php } ?>
	</section>
</div>