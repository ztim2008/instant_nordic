...
## 1) Что уже управляется глобально (через страницы Instant + Токены блоков)

Источники: form/model каталоги `landingbuilder` + формы `nordicbuilder` (`instant_global`, `tokens`).
...
Это список логичных улучшений страниц дизайн-системы (Instant + Токены), которые усилят контроль дизайнера без вмешательства в движок:
