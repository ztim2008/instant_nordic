...
- Следующий шаг:
	- при необходимости сделать финальный click-smoke именно по колонке и элементу на `Планшет/Телефон` (в этой итерации подтверждено переключение device-контекста и quick-actions).
