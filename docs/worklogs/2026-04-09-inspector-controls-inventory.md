...
## Update 2026-04-09 (P1 pass 4: alignment/source cleanup)

1. Поле колонки `settings.align` в базовом Inspector переименовано в более понятное `Расположение внутри колонки`.
2. Для block-элементов `source_key` подан как понятный выбор `Тип элемента` (вместо технического названия preset).
3. Технический `source_key` для non-block переведен в advanced-only режим (`lb_inspector_advanced=1`) и скрыт из базового профиля.
4. Короткий визуальный smoke подтверждает поведение на устройствах:
	- `Планшет` -> viewport `820px`, quick-actions показывают device-контекст `...на Планшет`;
	- `Телефон` -> viewport `430px`, quick-actions показывают device-контекст `...на Телефон`.

## Update 2026-04-09 (Phone grid responsiveness fix)

1. Исправлено поведение middle-зоны канвы в `Телефон`: сетка теперь реально перестраивается по выбранному `Body Layout` (`1/2-left/2-right/3`).
2. В section inspector добавлена подсказка для кейса с одной колонкой, чтобы убрать ложное ожидание «сетка не реагирует».
3. Browser-check подтвердил переключение: `1` -> одна колонка, `3` -> три колонки, `2-right` -> две колонки.
