...
## Закрытие дня (2026-04-07)

### Что сделано сегодня поверх pivot

1. Стабилизирован runtime для native body на внутренних страницах (включая fallback adapter/bindings и guard от захвата `content_body` секциями).
2. Доведены shell-настройки body frame (1/2/3 колонки + ширины сайдбаров) до live-рендера в теме.
3. Canvas переработан в zone-first UX вокруг body, упрощен inspector для новичков.
4. Исправлен регресс с дублирующимся `Body (системный, auto)` на холсте.

### Что проверено

1. `php -l` для измененных live/mirror PHP/TPL файлов — без ошибок.
2. parity `cmp` live vs mirrors по ключевым файлам — совпадает.
3. editor diagnostics по ключевым файлам — без новых ошибок.

### Что осталось на завтра

1. Ручной smoke guest/admin по маршрутам `/`, `/board`, профиль пользователя.
2. Проверка parity canvas/preview/live на тех же сценариях.
3. UX-полировка reorder по зонам и финальная фиксация checklist/документации.
