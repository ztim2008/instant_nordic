...
Дополнительное UX-правило первой design-manifest волны:

1. внутри вкладок панели рендерятся аккордеонами по сущностям и группам;
2. выбранная на canvas сущность должна автоматически раскрывать нужный аккордеон.
...
Для product-level design-manifest v1 на эти две сущности накладывается обязательное правило:

1. `title` и `subtitle` должны существовать в entity registry каждого блока;
2. для них обязателен visibility toggle;
3. при выключении элемент не занимает место в layout.
  "body": {
    "key": "body",
