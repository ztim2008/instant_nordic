...
Дополнительное правило для optional сущностей первой волны:

1. `meta` не является обязательной сущностью для каждого блока;
2. если блок поддерживает meta-строку, она должна жить отдельной сущностью `meta`, а не быть безымянным куском HTML;
3. `meta` должна получать собственную типографику и может скрываться целиком, если все её поля пустые.
