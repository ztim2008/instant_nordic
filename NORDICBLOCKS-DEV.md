...
2. inspector-поля `layout.desktop/mobile.paddingTop`, `paddingBottom`, `minHeight`, `contentGap` и `actionsGap` считаются рабочими layout-контролами Hero, а не декоративными полями без эффекта;
3. сущность `media` должна управлять самим изображением как минимум по `aspectRatio`, `objectFit` и `radius`, и эти значения должны доходить до live preview и SSR;
4. сущность `mediaSurface` должна управлять обрамлением медиа как минимум по `backgroundColor`, `padding`, `radius`, `borderWidth`, `borderColor` и `shadow`, и эти значения должны доходить до live preview и SSR;
5. если значение цвета текста не задано явно, Hero продолжает брать цвет из темы блока, а не ломает theme fallback;
6. Hero больше не должен опираться на block-scoped inline CSS для layout/media/typography: стандартом считается общий runtime-слой `blocks.css` + section-level CSS variables из render.
...
#### 🟢 Правило для следующих семейств блоков

После стабилизации `hero`-семейства новые продуктовые блоки больше не запускаем как случайные одиночные эксперименты.

Дальше действует family-first правило:

1. сначала выбираем мини-линейку из 3 близких блоков;
2. потом выбираем только один первый блок этой линейки для старта;
3. до кода фиксируем рабочее ТЗ первого блока;
4. только после этого начинаем реализацию.

Рабочее ТЗ для каждой новой линейки обязано заранее фиксировать:

1. пользовательские названия блоков на русском языке;
2. состав линейки и порядок rollout;
3. сущности блока;
4. поля по вкладкам `Контент`, `Дизайн`, `Макет`, `Данные`;
5. границы первой реализации;
6. что явно не входит в v1.

Это нужно, чтобы:

1. не плодить почти одинаковые блоки без общей системы;
2. не тащить в код лишние поля, которые потом придётся обслуживать вечно;
3. строить следующие семейства (`Новости`, `FAQ`, `Партнёры`, `Hero` и т.д.) по одному и тому же взрослому контуру.

#### 🟢 Следующее семейство после hero

Следующим продуктовым семейством после `hero` фиксируем `news/content-list` направление.
