...
if(currentValue.indexOf('vh')>=0){currentValue=currentValue.replace('vh','')}
if(currentValue.indexOf('vw')>=0){currentValue=currentValue.replace('vw','')}
if(currentValue!='0'&&currentValue!=''&&currentValue!='none'){$input.attr('placeholder',currentValue)}})});$unitsToggler.click(function(e){e.preventDefault();var $btn=$(this);var units=$btn.text();if(units=='px'){units='%'}else if(units=='%'){units='vh'}else if(units=='vh'){units='vw'}else if(units=='vw'){units='px'}
$btn.text(units);$input.keyup()});$wrap.appendTo($sizesBox)})});$.each(['margin','padding'],function(i,style){$.each(['','top','right','left','bottom'],function(j,dir){var rule=style+(dir?'-'+dir:'');var $wrap=$('<div/>').addClass('input-number').addClass('size-'+rule);var $input=$('<input/>').attr('type','text').addClass('input').addClass('input').data('rule',rule).attr('title',lang.styleNames[rule]);$input.keyup(function(e){var $input=$(this);var value=$input.val();var style=$input.data('rule');value=$.isNumeric(value)?value+'px':value;if(value===''){value=null}
...
if(currentValue.indexOf('vh')>=0){currentValue=currentValue.replace('vh','');currentValueUnits='vh'}
if(currentValue.indexOf('vw')>=0){currentValue=currentValue.replace('vw','');currentValueUnits='vw'}
...
if(value.indexOf('vh')>=0){value=value.replace('vh','');valueUnits='vh'}
if(value.indexOf('vw')>=0){value=value.replace('vw','');valueUnits='vw'}
...
delete this.options.placeholdWidth;delete this.options.placeholdHeight};$img.attr('src',this.options.src);$img.attr('alt',this.options.alt);$img.attr('title',this.options.title);if(this.options.url){var $link=$('<a></a>').attr('href',this.options.url).appendTo($block);$link.attr('target',this.options.target||'_self');$img.appendTo($link)}else if($img.parent('a').length>0){var $link=$img.parent('a');$img.appendTo($block);$link.remove()}};this.getStartupOptionsForm=function(){var formSchema=this.getOptionsForm();formSchema.tabs.placehold={title:lang.image.placehold,fields:{placeholdSource:{title:lang.image.placehold_src,type:'list',items:{'flat':lang.image.placehold_src_flat,'photo':lang.image.placehold_src_photo},default:'flat'},placeholdWidth:{type:'number',title:lang.image.width,},placeholdHeight:{type:'number',title:lang.image.height,},}};return formSchema};this.getOptionsForm=function(){var formSchema={title:lang.options+' - '+this.getTitle(),height:540,tabs:{basic:{title:lang.options,fields:{src:{type:'imageUrl',title:lang.image.src,default:url_tpl('images/img-default.png')},alt:{type:'string',title:lang.image.alt,},title:{type:'string',title:lang.image.title,},url:{type:'string',title:lang.link_url,},target:{type:'list',title:lang.link_target,items:{'_self':lang.link_targets.self,'_blank':lang.link_targets.blank,},default:'_self'}}}}};return formSchema};this.onCreate()};function InthemerBlockMenu(){InthemerBlock.apply(this,arguments);var block=this;var $block;this.getDefaultTitle=function(){return lang.blocks.menu};this.getType=function(){return'InthemerBlockMenu'}
...
this.isContainer=!0;this.isSortable=!0;var dataSource,dataItems;this.createBlock=function(){$block=$('<article></article>').addClass('page-article').addClass('empty');setTimeout(function(){block.onOptionsUpdate()},100);this.on(['child-added','child-removed','child-options-update','sort-end','sort-update'],function(){block.onOptionsUpdate()});this.on('sort-start',function(event){if(event.sender==block){block.removeClones()}});this.on('remove',function(event){block.removeClones()});return $block};this.addFieldBlockTo=function(targetBlock,targetLayout){var fieldListItems={};$.each(dataSource.fields,function(name,field){fieldListItems[name]=field.title});var formSchema={title:lang.add_field,width:350,height:300,tabs:{basic:{title:lang.options,fields:{fieldName:{type:'list',title:lang.add_field,items:fieldListItems,default:fieldListItems?Object.keys(fieldListItems)[0]:!1,},isLink:{type:'checkbox',title:lang.data_block.is_link,},isAddTitle:{type:'checkbox',title:'Добавлять заголовок поля'}}}}};var form=new InthemerForm(formSchema,{});form.on('submit',function(values){var fieldName=values.fieldName;var isLink=values.isLink;var isAddTitle=values.isAddTitle;var dataItem=dataSource.data[0];var block,field=dataSource.fields[fieldName];switch(field.type){case 'InthemerBlockDataText':block=new InthemerBlockDataText(!1,{text:dataItem[fieldName],isLink:isLink,isAddTitle:isAddTitle,url:isLink?dataItem._url:!1,fieldName:fieldName});block.setTitle(field.title);break;case 'InthemerBlockDataImage':block=new InthemerBlockDataImage(!1,{src:dataItem[fieldName],isLink:isLink,isAddTitle:isAddTitle,url:isLink?dataItem._url:!1,fieldName:fieldName});block.setTitle(field.title);break;case 'InthemerBlockDataBg':block=new InthemerBlockDataBg(!1,{src:dataItem[fieldName],fieldName:fieldName});block.setTitle(field.title);break;case 'InthemerBlockDataWrap':block=new InthemerBlockDataWrap(!1,{html:dataItem[fieldName],fieldName:fieldName,fieldType:field.srcType});block.setTitle(field.title);break}
...
this.options=options?$.extend({},options):{};this.getStyleSelectorPathsList=function(){return[{title:'Текст поля',name:'fieldtext',path:'.'+this.getCSSClass()},{title:'Заголовок поля',name:'fieldtitle',path:'.'+this.getCSSClass()+' .it-field-title'},]};this.createBlock=function(){if(this.options.isLink){$block=$('<a></a>').attr('href',this.options.url)}else{$block=$('<div></div>')}
...
$block.attr('data-field',this.options.fieldName);InthemerBlockDataTextUpdate($block,this)};this.getOptionsForm=function(){var formSchema={title:lang.options+' - '+this.getTitle(),height:530,tabs:{basic:{title:lang.options,fields:{isLink:{type:'checkbox',title:lang.data_block.is_link,},isAddTitle:{type:'checkbox',title:'Добавлять заголовок'},maxLength:{type:'number',title:lang.text.max_len}}}}};return formSchema};this.onCreate()}
...
$block.html(text);if(options.isAddTitle){$block.prepend('<span class="it-field-title">'+prototypeBlock.getTitle()+': </span>')}};function InthemerBlockDataImage(callback,options){InthemerBlock.apply(this,arguments);var $block,$img;this.getDefaultTitle=function(){return lang.blocks.image};this.getType=function(){return'InthemerBlockDataImage'}
