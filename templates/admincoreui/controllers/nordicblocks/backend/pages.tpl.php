...
<div class="nb-page-card">
    ...
    <div class="nb-page-card__actions">
        ...
        <?php endif; ?>
        <button
            class="nb-btn-sm nb-btn-sm--del nb-delete-page"
            data-id="<?= (int)$page['id'] ?>"
            data-title="<?= $title_esc ?>"
            data-url="<?= htmlspecialchars($delete_page_url, ENT_QUOTES, 'UTF-8') ?>"
...
<script>
document.querySelectorAll('.nb-delete-page').forEach(function(btn) {
    btn.addEventListener('click', function() {
        if (!confirm('Удалить страницу «' + btn.dataset.title + '»? Это действие нельзя отменить.')) return;
        btn.disabled = true;
